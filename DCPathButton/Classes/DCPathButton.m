...
- (void)configureViewsLayoutWithButtonSize:(CGSize)centerBtnSize
{
    ...
    /*
    UIToolbar* bgToolbar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, self.bloomSize.height/2+100, self.bloomSize.width * 2, self.bloomSize.height/2)];
    ...
    */
    ...
    //_bottomView.backgroundColor = [UIColor colorWithWhite:1.0 alpha:0.8];
    ...
    switch (self.bloomDirection) {
        case DCPathButtonBloomTop:
            return CGPointMake(self.pathCenterButtonBloomCenter.x - cosf(angel * M_PI) * itemExpandRadius,
                               (self.pathCenterButtonBloomCenter.y + 30) - sinf(angel * M_PI) * itemExpandRadius);
...
- (CAAnimationGroup *)foldAnimationFromPoint:(CGPoint)endPoint withFarPoint:(CGPoint)farPoint
{
    ...
    rotationAnimation.values = @[@(0), @(M_PI), @(M_PI * 2.0)];
...
- (void)pathCenterButtonBloom
{
    ...
    for (int i = 1; i <= self.itemButtons.count; i++) {
        ...
        CAAnimationGroup *bloomAnimation = [self bloomAnimationWithEndPoint:endPoint
                                                                ...
                                                                andNearPoint:nearPoint andView:pathItemButton];
...
- (CAAnimationGroup *)bloomAnimationWithEndPoint:(CGPoint)endPoint andFarPoint:(CGPoint)farPoint andNearPoint:(CGPoint)nearPoint andView:(DCPathItemButton*)pathItemButton
{
    CATransform3D perspective = CATransform3DIdentity;
    perspective.m34 = 1.0 / - 1000.0;
    pathItemButton.layer.sublayerTransform = perspective;
    
    
    CAKeyframeAnimation *scaleAnimation = [CAKeyframeAnimation animationWithKeyPath:@"transform"];
    
    CATransform3D startingScale = CATransform3DScale (pathItemButton.layer.transform, 0, 0, 0);
    CATransform3D overshootScale = CATransform3DScale (pathItemButton.layer.transform, 1.2, 1.2, 1.0);
    CATransform3D undershootScale = CATransform3DScale (pathItemButton.layer.transform, 0.9, 0.9, 1.0);
    CATransform3D endingScale = pathItemButton.layer.transform;
    ...
    //
    CAKeyframeAnimation *rotationAnimation = [CAKeyframeAnimation animationWithKeyPath:@"transform.rotation.z"];
    rotationAnimation.values = @[@(0.0),@(- M_PI * 2.8),@(-M_PI*2.0)];
    ...
    //
    CAAnimationGroup *animations = [CAAnimationGroup animation];
    animations.animations = @[movingAnimation, rotationAnimation,scaleAnimation];
